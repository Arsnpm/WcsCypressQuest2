...
describe('Authentication Tests suite preprod.backmarket', () => {
    ...
    it('should display error message for invalid password', () => {
        ...
        cy.contains("Au moins 8 caractères, dont 1 majuscule, 1 minuscule et 1 chiffre. Parce qu'on sait jamais.").should('exist');
        cy.get('[data-qa="signup-submit-button"]').click();     
    });
    ...
    it('should display error message for invalid fields', () => {   
        ...
        cy.get('#lastName').should('have.css', 'border-color', 'rgb(169, 15, 20)');
        cy.get('#signup-email').should('have.css', 'border-color', 'rgb(169, 15, 20)');
        cy.get('#signup-password').should('have.css', 'border-color', 'rgb(169, 15, 20)');
    ...
    it('should login user with valid credentials', () => {
        ...
        cy.get('[data-qa="accept-cta"]').click();
        cy.get('#signin-email').should('not.have.css', 'border-color', 'rgb(169, 15, 20)').type(userData.email).should('have.value', userData.email);
        cy.get('#signin-password').should('not.have.css', 'border-color', 'rgb(169, 15, 20)').type(userData.password);
